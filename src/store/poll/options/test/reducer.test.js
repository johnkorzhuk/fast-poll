...
describe('poll/options reducer', () => {
  ...
  describe('handles action creator', () => {
    ...
    it('addOptionAction as expected', () => {
      expect(reducer(undefined, actions.addOptionAction('1'))).toMatchSnapshot()
    })
    ...
    it('updateOptionAction as expected', () => {
      expect(reducer(state, actions.updateOptionAction('1', { editing: true, text: '1 updated', votes: 1 })).data['1']).toMatchSnapshot()
    })

    it('updateOptionOrderAction as expected', () => {
      expect(reducer(state, actions.updateOptionOrderAction(['3', '2', '1'])).order).toMatchSnapshot()
    })

    it('resetOptionsAction as expected', () => {
      expect(reducer(state, actions.resetOptionsAction())).toEqual(INITIAL_STATE)
    })
  })
})